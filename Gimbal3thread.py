...
def RunGimbal():
    global mode, tilt, period, roll, pitch
    ...
    while 1:
        ...
        print("Servo 1: %f" %ServoAng1)
        print("Servo 2: %f" %ServoAng2)
        print(" ")   
...
def getIMU3():
    global init3, pitch_out, roll_out, roll, pitch
    Dt=0.01
    while 1:
        acc_data = sensor.get_accel_data()
        gyro_data=sensor.get_gyro_data()
        ...
        print("Roll : %f" %roll_out)
        print("Pitch : %f" %pitch_out)
        sem2.acquire()
        roll = roll_out    
        pitch = pitch_out
        sem2.release()
...
try:
   ...
   t3 = threading.Thread(target = getIMU3)
   t1.start()
   t2.start()
   t3.start()
